...
class VerificationPhoneNumberInteractor {
   func verifyPhoneNumber(phoneNumber: String,
    ...
    func verifyPassword(phoneNumber: String,
                 userType: String,code : Int,
                  completionHandler: @escaping (String?, Error?) -> Void) {
        
     AF.request(SehaKhanahRouter.verifyPassword(phonenumber: phoneNumber, userType: userType, code:code )).responseJSON {
            (response) in
            ...
            case .success:
                ...
                print(json)
                if json["success"] == true{
                    let status = json["data"]["token"].stringValue
                    completionHandler(status, nil)
                }
            case .failure(let error):
                completionHandler(nil, error)
