...
class OffersInteractor {
    ...
    func offerSlidShow(
             completionHandler: @escaping ([MostOrderedOffersImage]?, Error?) -> Void) {
          AF.request(SehaKhanahRouter.offerSlidShow).validate().responseJSON{
                 (response) in
                 ...
                 case .success :
                     let json = JSON(response.value)
                     print(json)
                     var mostOrderedOffersImageList = [MostOrderedOffersImage]()
                     let mostOrderedOffersImages = json["data"].arrayValue
                     
                     for mostOrderedOfferImage in mostOrderedOffersImages
                     {
                        let data = MostOrderedOffersImage(withJson: mostOrderedOfferImage)
                        mostOrderedOffersImageList.append(data)
                         
                     }
                     completionHandler(mostOrderedOffersImageList, nil)
                 case .failure(let error):
                     completionHandler(nil, error)
                 }
